...
function ActionLink({
  ...
  if (variant === "secondary") {
    return (
      <Link
        href={href}
        className={`${baseClassName} border border-black/10 bg-white text-neutral-900 duration-200 hover:border-[#0477BF] hover:bg-white hover:shadow-[0_0_0_3px_rgba(4,119,191,0.15)]`}
      >
        {children}
      </Link>
    );
  ...
  return (
    <Link
      href={href}
      className={`${baseClassName} bg-neutral-950 text-white duration-200 hover:bg-[#0477BF] hover:text-white hover:shadow-[0_0_0_3px_rgba(4,119,191,0.15)]`}
    >
      {children}
    </Link>
  );
...
export function CartSummaryCard({ summary }: Props) {
  ...
  return (
    <aside className="rounded-[28px] border border-black/10 bg-white p-5 shadow-sm sm:p-6">
      ...
      {summary.mixedFlow ? (
        <div className="mt-5 rounded-2xl bg-[#0477BF]/10 px-4 py-3 text-sm leading-6 text-[#04395c]">
